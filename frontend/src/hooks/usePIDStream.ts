...
import { usePIDStore, PIDReading } from '../store/pidStore'
...
export function usePIDHistory() {
  const historyRef = useRef<Record<string, PIDReading[]>>({})
  ...
  return {
    getHistory: (pid: string): PIDReading[] => historyRef.current[pid] ?? [],
  }
}
