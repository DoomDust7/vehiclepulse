    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {recentFault && (
          <span style={{
            ...
            {recentFault.severity.toUpperCase()}
          </span>
        )}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
        <span style={{ fontSize: 28, fontWeight: 700, color, fontVariantNumeric: 'tabular-nums' }}>
          {value !== null ? value.toFixed(value > 99 ? 0 : 1) : '—'}
        </span>
      ...
      <div style={{ height: 4, background: '#2a2a2a', borderRadius: 2 }}>
        <div style={{ height: '100%', width: `${pct * 100}%`, background: color, borderRadius: 2, transition: 'width 0.1s' }} />
      </div>

      <Sparkline history={history} min={meta.min} max={meta.max} color={color} />
    </div>
  )
}
